...
<div class="breadcrumbs">
    {{ Breadcrumbs::render('almacen.edit') }}
    </div>
...
@include('right_sidebar')
        ...
        <form method="POST" action="{{route("almacen.update", ['id_almacen'=>$almacen->id])}}">
            @method("PUT")
            @csrf
            <div class="row">
                    <div class="col-sm-6">
                ...
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <input required autocomplete="off" name="direccion" class="form-control"
                           type="text" placeholder="direccion" value="{{$almacen->direccion}}">
                    </div>
                    <div class="col-sm-6">
                        <label class="label">Whastapp</label>
                        <input required autocomplete="off" name="whastapp" class="form-control"
                           type="text" placeholder="whastapp" value="{{$almacen->whastapp}}">
                    </div>
                    ...
                    <div class="col-sm-6">
                        ...
                        <select required multiple name="vendedores[]" class="form-control" id="vendedores">
                            @foreach($vendedores as $vendedor)
                                <option value="{{$vendedor['id']}}" {{ $vendedor['seleccionado'] ? 'selected' : '' }}>{{$vendedor['name']}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
